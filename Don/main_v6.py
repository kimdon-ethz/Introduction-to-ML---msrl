...
#preprocessor = ColumnTransformer(transformers = [('vital', itt, vital)])
#sc = StandardScaler()

#Imputing missing values and drop unwanted features
#X_i = df.copy()
...
model = Sequential()
model.add(Dense(256, input_dim=287, activation='relu'))
...
for i in ['LABEL_Sepsis']:
    model.fit(X_trains, to_categorical(Y_train[i], 2), epochs=3, batch_size=128, \
          validation_data=(X_vals, to_categorical(Y_val[i], 2)))
